            ...
            if (s_a_have_b(s, *i, serv_type[o]))
            {
                *i += serv_type[o].length();
                ...
                case (2): //root
                    ...
                    if (serv_tmp->lvl && tmp.substr(0, tmp.length() - (tmp[tmp.length() -1] == '/' ? 1 : 0)) == serv_tmp->root)
                ...
                case (10): //rewrite
                    ...
                    for (std::vector<std::pair<std::string, std::string> >::iterator it = serv_tmp->redirect.begin(); it != serv_tmp->redirect.end(); ++it)
                        if (it->first == tmp)
                            return (error_config_message(s, *line_i, 26) + 1);
...
bool    Config::get_conf(const std::string s)
{
    ...
    while (1)
    {
        ...
        if (init_location(server[server.size() - 1], &(server[server.size() - 1].locations)))
            return 1;
        i_serv++;
        vp.clear(); 
        i_loc = 1;
...
void    Config::init_loc_tmp(Server *dest, Server src)
{
    bool b = 0;
    if (dest->root.empty())
        dest->root = src.root;
    ...
    if (dest->error_page.empty())
        dest->error_page = src.error_page;
    if (dest->client_max_body_size.empty())
        dest->client_max_body_size = src.client_max_body_size;
    if (dest->cgi.empty())
        dest->cgi = src.cgi;
    // if (!dest->autoindex)init a la creation
        // dest->autoindex = src.autoindex;
    for (std::vector<std::pair<std::string, std::string> >::const_iterator src_it = src.redirect.begin(); src_it != src.redirect.end(); src_it++)
    {
        b = 0;
        for (std::vector<std::pair<std::string, std::string> >::iterator dest_it = dest->redirect.begin(); dest_it != dest->redirect.end(); dest_it++)
        {
            if (dest_it->first == src_it->first)
            {
                b = 1;
                break;
            }
        ...
        if (!b)
            dest->redirect.push_back(*src_it);
    }
}

bool    Config::init_location(Server s, std::map<std::string, Server*>* m)
{
    for (std::vector<Server>::iterator it = s.loc.begin(); it != s.loc.end(); it++)
    {
        if (m->count(it->root + it->path))
        {
            std::cerr << "Error config: server " << s.loc_id[0] << ": can't have two location with same absolut path(" << it->root << it->path << ")." << std::endl;
            return 1;
        }
        m->insert(std::make_pair(it->root + it->path, &(*it)));
        if (init_location(*it, m))
            return 1;
    }
    return 0;
}
...
std::ostream&	operator<<(std::ostream& ostream, const Server& src)
{
    ...
    for (std::vector<Server>::const_iterator it = src.loc.begin(); it != src.loc.end(); it++)
        ostream << *it;
